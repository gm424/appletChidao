...
const dateTransformer = data => {
  // 需要同时判断Date类型和Moment类型
  if (data instanceof Date || (data && data._isAMomentObject)) {
    console.log('dateTransformer', data)
  ...
  if (typeof data === "object" && data !== null) {
    return _.fromPairs(Object.entries(data).map(([ key, val ]) =>
...
service.interceptors.response.use((response) => {
  ...
  if (code === 500) {
    Vue.prototype.$Jnotification.error({message: '系统提示', description: message})
  }
